...
class SimpleCalendar:
    ...
    async def process_calendar_selection(self, query: CallbackQuery, data: CallbackData) -> tuple:
        """
        ...
        if data['act'] == "PREV-YEAR":
            ...
            await query.message.edit_reply_markup(await self.create_calendar(int(prev_date.year), int(prev_date.month)))
        # user navigates to next year, editing message with new calendar
        if data['act'] == "NEXT-YEAR":
            next_date = temp_date + timedelta(days=365)
            await query.message.edit_reply_markup(await self.create_calendar(int(next_date.year), int(next_date.month)))
